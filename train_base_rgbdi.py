...
def parse_option():
    ...
    # dataset
    parser.add_argument('--dataset', type=str, default='train100', choices=['train100', 'train5', 'train25', 'train50'])
...
def set_model(args):
    ...
    return model_x, model_y, model_z, criterion
...
def train(epoch, train_loader, model_x, model_y, model_z, \
            criterion, optimizer_x, optimizer_y, optimizer_z, \
            opt):
    """
    one epoch training
    """
    ...
    for idx, (inputs, index) in enumerate(train_loader):
        ...
        # ===================forward=====================
        # with torch.no_grad():
        feat_x, output_x = model_x(input_x) # [bs, 8, 512]
        feat_y, output_y = model_y(input_y) # [bs, 8, 512]
        feat_z, output_z = model_z(input_y) # [bs, 8, 512]
    ...
    return top1_x.avg, top5_x.avg, losses_x.avg, \
            top1_y.avg, top5_y.avg, losses_y.avg, \
             top1_z.avg, top5_z.avg, losses_z.avg


def validate(val_loader, model_x, model_y, model_z, \
            criterion, opt):
    """
    evaluation
    """
    ...
    with torch.no_grad():
        ...
        for idx, (inputs, index) in enumerate(val_loader):
            ...
            feat_x, output_x = model_x(input_x) # [bs, 8, 512]
            feat_y, output_y = model_y(input_y) # [bs, 8, 512]
            feat_z, output_y = model_z(input_z) # [bs, 8, 512]
...
def main():
    ...
    # set the model
    model_x, model_y, model_z, criterion = set_model(args)
    ...
    optimizer_x = set_optimizer(args, model_x)
    optimizer_y = set_optimizer(args, model_y)
    optimizer_z = set_optimizer(args, model_z)
    ...
    if not args.test:
        ...
        for epoch in range(args.start_epoch, args.epochs + 1):
            ...
            time1 = time.time()
            top1_x, top5_x, losses_x, top1_y, top5_y, losses_y, top1_z, top5_z, losses_z = \
                                                        train(epoch, train_loader, model_x, model_y, model_z, \
                                                        criterion, optimizer_x, optimizer_y, optimizer_z, args)
            time2 = time.time()
            ...
            print("==> evaluating...")
            top1_x, top5_x, losses_x, top1_y, top5_y, losses_y, top1_z, top5_z, losses_z = \
                                                        validate(eval_loader, model_x, model_y, model_z, criterion, args)
            ...
            if epoch % args.save_freq == 0:
                ...
                save_name = 'rgbdi_ckpt_epoch_{epoch}.pth'.format(epoch=epoch)
                save_name = os.path.join(args.save_folder, save_name)
        ...
        print("==> testing...")
        validate(test_loader, model_x, model_y, model_z, \
