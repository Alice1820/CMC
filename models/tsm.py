...
class TSMV1(nn.Module):
    def __init__(self, name='resnet50', num_segments=8):
        super(TSMV1, self).__init__()
        if name == 'resnet50':
            self.l_to_ab = TSN(num_segments=num_segments)
            self.ab_to_l = TSN(num_segments=num_segments)
        # elif name == 'resnet18':
...
class MyTSMCMC(nn.Module):
    def __init__(self, name='resnet50v1', num_segments=8):
        super(MyTSMCMC, self).__init__()
        if name.endswith('v1'):
            self.encoder = TSMV1(name[:-2], num_segments=num_segments)
